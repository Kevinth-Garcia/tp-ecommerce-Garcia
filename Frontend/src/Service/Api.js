...
async function apiRequest(endpoint, options = {}) {
  //una funcion para hacer las peticiones HTTP
  try {
    const res = await fetch(`${API_URL}${endpoint}`, options);
    if (!res.ok) {
      //si la respuesta no es OK entonces que mande a la pagina 404//
    ...
    //uso para atrapar cualquier error ya sea de red, conexion, o parcing.
...
export const createOrder = (order) =>
  apiRequest("/orders", {
    //se crea la orden en el cual es enviado al JSON en el cuerpo del request agregandolo al apartado orders//
    method: "POST",
